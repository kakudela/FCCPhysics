...
`run_gp_timing.sh`: Automatically runs different Tracking/noTracking and gridsN of Guinea-Pig for
ease because I'm lazy, writes timing information in a csv that is analyzed and plotted by another script.

`plot_gp_timings.py`: Takes the CSV that was written after the timing studies, plots number of GP grids
vs. computing time
