...
def get_hist(proc, norm=False, lumi=150e6):
    fIn = ROOT.TFile(f"output/tutorials/z_mumu_afb/{proc}.root")
    hist = fIn.Get("invariant_mass")
    xsec = fIn.Get("crossSection").GetValue()
    nevents = fIn.Get("eventsProcessed").GetValue()
    if norm:
...
if __name__ == "__main__":
    ...
    for iBin in range(1, h_costhetac.GetNbinsX()+1):
        ...
        g_costhetac.SetPointError(iBin-1, 0, y**0.5) # set the bin error equal to the data statistics
