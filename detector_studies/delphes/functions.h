...
Vec_f leptonResolution(Vec_rp leptons, Vec_i recind, Vec_i mcind, Vec_rp reco, Vec_mc mc, int mode=0) {
    ...
    for(int i = 0; i < leptons.size(); ++i) {
        ...
        if(mc_index >= 0 && mc_index < (int)mc.size()) {
            ...
            if(mode == 0) result.push_back((reco_.P()-mc_.P())/mc_.P());
            else if(mode == 1) result.push_back((reco_.Theta()-mc_.Theta())/mc_.Theta());
            else if(mode == 2) result.push_back((reco_.Phi()-mc_.Phi())/mc_.Phi());
        }
    }
    return result;
}
